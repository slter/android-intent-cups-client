...
public class CupsGetPrintersOperation extends IppOperation {
	...
	URL requestURL = new URL(url.toString() + path);
	L.w("CupsGetPrintersOperation::getPrinters(): request from URL :" +
	    requestURL.toString());
        IppResult result = request(requestURL, map);
	L.w("CupsGetPrintersOperation::getPrinters(): IPP result is " + result);
        if (result == null) {
